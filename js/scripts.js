...
//             el.getNiceScroll().resize();
    ...
    $(document).on('click','.js-accordeon_head', function(){
        var acc_head = $(this);
        if( acc_head.parents('.many-accordeon').size() > 0 ){
            console.log('kjfs');
            var acc_block = acc_head.parent('.js-accordeon'),
                acc_body = acc_block.children('.js-accordeon_body');
        }else{
            var acc_block = acc_head.parents('.js-accordeon'),
    ...
    $(document).on('change','.shop-filtr input[type="checkbox"]', function(){
        ...
        else if( check.parents('.js-accordeon_body').size() > 0 ){
    ...
    $(document).on('click', '.school-progress_table tbody td', function() {
